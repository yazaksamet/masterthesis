...
completeData$timestampX = strptime(c("1.1.2000 00:00"), format = "%d.%m.%Y %H:%M", tz = "CET") + completeData$timestamp
...
for (i in 99:99) {
  ...
  res = AnomalyDetectionTs(testData, plot=TRUE, longterm = TRUE)
